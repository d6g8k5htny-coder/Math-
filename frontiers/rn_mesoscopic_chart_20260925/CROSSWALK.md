...
|---|---|---|
| (1) Independent contact rows `J_0(y)` after pin constraints | **Partial** — enumerated on `C_transverse`, `C_axial`, and `C_thin_belt` (shared jet helper); pin-site **leading Morse + cubic through undecic** rows + max/saddle Hessian signature; twelfth-and-higher pin jets open | `mesoscopic_chart.py` contact / thin-belt / axial / `pin_site_morse_contact_rows` / `pin_site_morse_next_order_rows` / `pin_site_morse_quartic_rows` / `pin_site_morse_quintic_rows` / `pin_site_morse_sextic_rows` / `pin_site_morse_septic_rows` / `pin_site_morse_octic_rows` / `pin_site_morse_nonic_rows` / `pin_site_morse_decic_rows` / `pin_site_morse_undecic_rows` / `pin_morse_hessian_signature` |
...
Small-A (`A≤1/2`) is a separate diagnostic regime: pin-local frame, leading Morse + cubic through undecic rows, max/saddle Hessian signature test, and obstruction ledger recorded; twelfth-and-higher pin jets and density open.
