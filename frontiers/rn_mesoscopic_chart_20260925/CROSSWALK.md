...
|---|---|---|
| (1) Independent contact rows `J_0(y)` after pin constraints | **Partial** — enumerated on `C_transverse`, `C_axial`, and `C_thin_belt` (shared jet helper); pin-site **leading Morse + cubic + quartic** rows + max/saddle Hessian signature; fifth-and-higher pin jets open | `mesoscopic_chart.py` contact / thin-belt / axial / `pin_site_morse_contact_rows` / `pin_site_morse_next_order_rows` / `pin_site_morse_quartic_rows` / `pin_morse_hessian_signature` |
...
| (3) Conditioned witness Hessian in the same chart | **Partial** — contact Hessian polynomials recorded; free-jet residual inventory after gradient contact recorded; conditioned Gaussian expectation **open** | `hessian_contact_rows` / `axial_hessian_contact_rows` / `contact_free_jet_residual_inventory`; `hessian_ledger_evaluated=false` |
| (4) Combine typed Hessian with gradient-density Jacobian | **Partial** — exact power identity only (`spatial - grad_jac + hess_det + height_window`); unmatched height `r^1` recorded; free-jet residuals enumerated; density factor unbound; per-chart obstruction inventory | net `r^3` transverse / pin / `r^2` axial; `transverse_height_r_factor_ledger`; `contact_free_jet_residual_inventory`; `contact_density_obstruction_inventory`; `contact_density_bound_proved=false` |
