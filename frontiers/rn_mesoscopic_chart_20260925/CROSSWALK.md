...
| (5) Local integrability across chart boundaries; uniform bound on `A≤\|y\|≤B` | **Partial** — `|y2|=δ` transition is identity (det 1); thin-belt interior still open (`1/\|y2\|` bare-L1 failure; jet-map pointwise cancel recorded, density unbound) | `chart_boundary_transition`, `thin_belt_reciprocal_shell_lower_bound`, `jet_map_f_yy_to_J_grad_y_factor` |
