...
| (4) Combine typed Hessian with gradient-density Jacobian | **Partial** — exact power identity only (`spatial - grad_jac + hess_det + height_window`); unmatched height `r^1` recorded; density factor unbound; per-chart obstruction inventory | net `r^3` transverse / pin / `r^2` axial; `transverse_height_r_factor_ledger`; `contact_density_obstruction_inventory`; `contact_density_bound_proved=false` |
