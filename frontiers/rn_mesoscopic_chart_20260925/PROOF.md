...
- pin-site sextic residuals `S_grad_next = (1/120)D⁶f(z,z,z,z,z)`, `S_height_next = (1/720)D⁶f(z,z,z,z,z,z)` with `z·S_grad_next = 6 S_height_next` (seventh-and-higher jets open);
...
- higher-order pin-site divided-difference jets beyond sextic residuals;
...
with scalings `p_grad=1`, `p_height=2` (gradient Jacobian `r`-power `2`), plus an exact Sylvester signature test (`M` ⇒ negative definite, `S` ⇒ indefinite), plus cubic/quartic/quintic/sextic residuals (unmatched `r^1`/`r^2`/`r^3`/`r^4`). Seventh-and-higher pin-site jets and the contact density remain open (`pin_site_higher_jets_enumerated = false`). This does not alter the PR7 fixed-annulus statements.
