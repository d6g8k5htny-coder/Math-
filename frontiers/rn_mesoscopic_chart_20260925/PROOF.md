...
- pin-site tridecic residuals `G_grad_next = (1/479001600)D¹³f(z^12)`, `G_height_next = (1/6227020800)D¹³f(z^13)` with `z·G_grad_next = 13 G_height_next` (fourteenth-and-higher jets open);
...
- higher-order pin-site divided-difference jets beyond tridecic residuals;
...
with scalings `p_grad=1`, `p_height=2` (gradient Jacobian `r`-power `2`), plus an exact Sylvester signature test (`M` ⇒ negative definite, `S` ⇒ indefinite), plus cubic through tridecic residuals (unmatched `r^1`…`r^11`). Fourteenth-and-higher pin-site jets and the contact density remain open (`pin_site_higher_jets_enumerated = false`). This does not alter the PR7 fixed-annulus statements.
