...
- pin-site nonic residuals `N_grad_next = (1/40320)D⁹f(z,z,z,z,z,z,z,z)`, `N_height_next = (1/362880)D⁹f(z,z,z,z,z,z,z,z,z)` with `z·N_grad_next = 9 N_height_next` (tenth-and-higher jets open);
...
- higher-order pin-site divided-difference jets beyond nonic residuals;
...
with scalings `p_grad=1`, `p_height=2` (gradient Jacobian `r`-power `2`), plus an exact Sylvester signature test (`M` ⇒ negative definite, `S` ⇒ indefinite), plus cubic through nonic residuals (unmatched `r^1`…`r^7`). Tenth-and-higher pin-site jets and the contact density remain open (`pin_site_higher_jets_enumerated = false`). This does not alter the PR7 fixed-annulus statements.
