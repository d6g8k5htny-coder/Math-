...
MUTANTS={
    'missing_axis_column':('two_scale.py','p.mul(a,alpha),p.mul(p.u,beta),{}','{},p.mul(p.u,beta),{}'),
    'missing_outer_column':('two_scale.py','[{},p.mul(b,alpha),beta]','[{},p.mul(b,alpha),{}]'),
    'unshifted_gap':('two_scale.py','p.constant(Q(-1,4))','p.constant(Q(0))'),
    'wrong_quartic_coefficient':('two_scale.py','p.scale(p.mul(p.u,gap),Q(1,6))','p.scale(p.mul(p.u,gap),Q(1,3))'),
    'wrong_count_power':('two_scale.py','return -3-7*gamma','return -2-7*gamma'),
    'wrong_height_contact_coefficient':('annulus.py','Q(-1,12)','Q(-1,6)'),
    'unsafe_cutoff_margin':('annulus.py','return 1-12*q','return 1-6*q'),
    'dropped_window':('annulus.py','height_window=3','height_window=0'),
    'dropped_original_normalizer':('annulus.py','normalizer=-2','normalizer=0'),
}
...
def main():
    ...
    for mode,flags in [('normal',[]),('optimized',['-O'])]:
        ...
        for name,mutation in [('baseline',None),*MUTANTS.items()]:
            with tempfile.TemporaryDirectory() as temporary:
                work=Path(temporary)
                for filename in ('thin_tube.py','test_thin_tube.py','two_scale.py','test_two_scale.py','annulus.py','test_annulus.py'):
                ...
                if mutation:
                    ...
                    (work/filename).write_text(mutated)
                ...
                require('Ran 40 tests' in log and 'skipped=' not in log,'coverage mismatch '+name)
                if mutation:
        ...
        for module in ('thin_tube.py','two_scale.py','annulus.py'):
    ...
    report={'object':'D5-FIXED-ANNULUS-STITCH-20260925-v1','python':sys.version,'distinct_tests':40,
            'old_tests':28,'new_tests':12,'semantic_mutants':len(MUTANTS),'modes':['normal','optimized'],
