...
**Object:** D1-SECTION9-BOREL-REPAIR-20260925-v1  
...
- Theorem 2.2: the Gaussian unweighted Kac–Rice formula under `C^1` paths and positive-definite point covariance; compact domains have finite expected level-set measure.
...
Fix a compact ordered-pair domain
[
Dsubset{(x,y)in X	imes X:x
e y}
]
with positive distance from the diagonal. Write
[
G(x,y)=(
abla f(x),
abla f(y))inmathbb R^{2d}.
]
By the positive Fourier spectrum and distinct-site distribution argument of parent Section 2, (operatorname{Cov}G(x,y)) is positive definite at every pair, uniformly on (D). The sample field is smooth. The derivative of (G) with respect to ((x,y)) is block diagonal, so its normal Jacobian is
[
...
Let (mathcal E=C^2(X)), equipped with its usual separable Banach topology on the compact torus. For (t=(x,y)in D) and (zinmathbb R^{2d}), finite-dimensional Gaussian regression gives a smooth conditional mean (m_{t,z}) and a centered residual Gaussian field (R_t) such that, as a random element of (mathcal E),
[
fmid G(t)=z;overset{d}=m_{t,z}+R_t .
]
The residual can be constructed from the full Gaussian field by subtracting
[
operatorname{Cov}(f,G(t))operatorname{Cov}(G(t))^{-1}G(t).
]
...
[
K(t,z,A)=mathbb P{m_{t,z}+R_tin A},qquad Ainmathcal B(mathcal E),
]
is a regular conditional probability kernel on (mathcal E), and for every bounded continuous cylinder functional (F), ((t,z)mapstoint F(phi)K(t,z,dphi)) is continuous. This is the whole-field version required below; it is not merely a family of conditional laws for a finite list of cylinder coordinates.

#### 9.2 Two finite measures on (D	imesmathcal E)

On the almost-sure zero set of (G), define
[
mu(A)=mathbb Esum_{tin D:G(t)=0}mathbf 1_A(t,f),
qquad Ainmathcal B(D	imesmathcal E).
]
Theorem 2.2 makes (mu) finite.

Define a second finite Borel measure
[

u(A)=int_D p_{G(t)}(0),
 mathbb E!left[Delta(t)mathbf 1_A(t,f)mid G(t)=0ight]dt.
]
The conditional expectation is defined by the kernel (K(t,0,cdot)). Its total mass equals the finite unweighted Kac–Rice integral, so (
u) is finite.
...
Choose a countable dense set (Qsubset X). The maps
[
phimapsto partial^alphaphi(q),
qquad qin Q,quad |alpha|le2,
]
together with a countable coordinate basis on (D), separate points of (D	imesmathcal E). Because the (C^2) norm on the compact torus is recovered from suprema over the dense set for derivatives through order two, these evaluations generate (mathcal B(D	imesmathcal E)).

Let (mathcal H) be the bounded real Borel functions (h) for which (9.1) holds. Since (mu,
u) are finite, (mathcal H)

1. is a vector space,
2. contains the constants,
3. is closed under bounded pointwise convergence by dominated convergence.

It contains the algebra of bounded continuous cylinder functions. The functional monotone-class theorem therefore implies that (mathcal H) contains **every bounded Borel function on (D	imesmathcal E)**.
...
[
(t,phi)mapsto mathbf 1{d_phi(x)=phi(y)}
]
is Borel on the Morse distinct-critical-value locus, with the essential-maximum convention preserved. Hessian index indicators are Borel functions of the (2)-jet. Their product with the elder mark is therefore a bounded Borel weight on (D	imesmathcal E), so (9.1) applies.
...
Disintegrate the two endpoint heights using their nondegenerate joint Gaussian density conditional on (G(t)=0). At prescribed heights (f(M)=b), (f(S)=b-kr^3), the Jacobian (Delta=|det H_Mdet H_S|) and the type indicators give exactly the parent endpoint weight (W_r). Thus candidate pair intensity contains the full-pin density times (mathbb E_Q W_r); the selected intensity contains the same factor times the (Q^W) global elder probability (p_r). There is no second determinant factor.
...
**Still separate:** contact covariance, residual-field independence in parent Section 4, typed boundary integration, full normalizer, Theorem A, off-diagonal/unbounded-mark domination, coefficient (15.2), numerical constants, RN/24-jet certificates, and every authoritative scientific-status gate.
...
- C3: finiteness and equality of (mu,
u) on continuous cylinders;
